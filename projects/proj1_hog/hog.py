...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        temp = dice()
        is_sow_sad = is_sow_sad or temp == 1
        sum += temp
        num_rolls -= 1
    if is_sow_sad:
        sum = 1
...
def picky_piggy(score):
    ...
    if digit == 0:
        return 7
    return int(seed * pow(10, digit) % 10)
    # END PROBLEM 2
...
def make_averaged(original_function, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def average(*args):
        # 不修改trials_count就不会出问题, 否则出现local variable reference before assignment error
        # in Python, you aren't normally allowed to modify variables defined in parent frames.
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_result = 0
    while n <= 10:
        num = make_averaged(roll_dice, trials_count)(n, dice)
        if num > max_result:
            max_result_roll = n
            max_result = num
        n += 1
    return max_result_roll
